...
public class FeedController {
    ...
    @Operation(summary = "피드 조회 메서드", description = "포스트된 모든 피드들을 확인할 수 있습니다.")
    @GetMapping
    public ResponseEntity<Page<FeedInfoResponse>> findAllFeed(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
            @RequestParam(defaultValue = "") String keyword
    ) {
        Pageable pageable = PageRequest.of(page, size, Sort.by("createdAt").descending());
        return ResponseEntity.ok(feedService.getFeedList(pageable, keyword));
    }
    ...
    @GetMapping("/my")
    public ResponseEntity<Page<FeedInfoResponse>> findMyFeed(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "10") int size,
